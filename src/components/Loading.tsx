...
const getBalls = ({
    ...
    ballSize,
    sizeUnit = 'px'
}: {countBalls: number, radius: number, angle: number, color: string, size: number, ballSize: number, sizeUnit: string}) => {
    const balls = [];
...
const Loading: React.FunctionComponent<{size?: number, color?: string, sizeUnit?: string}> = ({ size = 40, color = '#fff', sizeUnit = 'px' }): JSX.Element => {
    const radius = size / 2;
    const countBalls = 9;
