...
import ImageShare from '../assets/imgs/share.svg';
...
const ShareButton = styled<any>(StyledButton)`
...
const Modal: React.FunctionComponent<{visible: boolean}> = ({ visible = false }): JSX.Element => {
    ...
    useEffect(() => {
        const qrEle: HTMLElement = document.querySelector('#QR_DOWNLOAD');
        ...
        <StyledModal className={visible ? 'visible' : 'hidden'}>
            ...
            <div className="qr hidden" id="QR_DOWNLOAD">
                <div className="img">
                    <CodeQR
                        renderAs="svg"
                        imageSettings={{ width: 30, height: 30, src: ImageLogoPath, excavate: true }}
                        size={256}
                        bgColor="#fff"
                        level="Q"
                        fgColor="#000"
                        includeMargin={true}
                        value={`${window.location.href.split('?')[0]}?idx=${(window as any).CUR_WORDS_IDX}`}
                    />
...
const ShareQR: React.FunctionComponent = (): JSX.Element => {
