...
import ImageLover from '../assets/imgs/lover.bg.png';
import ImageBirds from '../assets/imgs/birds.bg.png';

import { AniPopIn, AniFadeIn, AniSlideInDown } from './animates';
import StyledWordBox from './StyledWordBox';
import ImageHeart from '../assets/imgs/heart.svg';
import ImageNoise from '../assets/imgs/noise.bg.png';

const Words = require('../assets/words.json');

const ImageLogoPath = '../assets/imgs/logo.png';
...
const Card: React.FunctionComponent<{visible: boolean}> = ({ visible = false }): JSX.Element => {
    ...
    useEffect(() => {
        if (visible) {
            ...
            // 全局变量
            (window as any).CUR_WORDS_IDX = Words.findIndex((w: string) => w === newWords);
        ...
        <StyledWrapper className={visible ? 'visible' : 'hidden'}>
            <div id="HONEYED_WORDS_CARD" className={`card`}>
                ...
                <div className="dbg qr">
                    <CodeQR
                        renderAs="svg"
                        imageSettings={{ width: 10, height: 10, src: ImageLogoPath, excavate: true }}
                        size={50}
                        bgColor="#f2f2f2"
                        level="Q"
                        fgColor="#000"
